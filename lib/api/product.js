...
import { getLanguageByIsoCode } from "./languages";
import { getTaxRuleGroups } from "./taxes";

export async function getProduct(idProduct, idProductAttribute = 0, languageId = 1) {
  const response = await fetchAPI(
    `products/${idProduct}`,
    `price[price_wt][use_tax]=1&price[price_wt][product_attribute]=${idProductAttribute}&language=${languageId}`
  );
...
export async function getCategoryProducts(idCategory, languageIsoCode) {
  ...
  if (response.category.associations.products) {
    products = await Promise.all(
      response.category.associations.products.map(async product => {
