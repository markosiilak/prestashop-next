...
import { API_CURRENCY, API_LANGUAGES, fetchAPI, postAPI, prestaEvents, putAPI } from "./api";
...
export async function initCart() {
  let userCart = "";
  if (user.id_customer != "") userCart = `<id_customer>${user.id_customer}</id_customer>`;
  else if (user.id != "") userCart = `<id_guest>${user.id}</id_guest>`;
  ...
  const results = await postAPI(
    ...
    `<?xml version="1.0" encoding="UTF-8"?>
        <prestashop xmlns:xlink="http://www.w3.org/1999/xlink">
            <cart>
                <id_currency>${API_CURRENCY}</id_currency>
                <id_lang>${API_LANGUAGES}</id_lang>
                ${userCart}
            </cart>
        </prestashop>`
  );
